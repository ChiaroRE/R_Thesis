...
EUROSTOX_data = read.csv("EUROSTOXX50, 01-01-2020 to 31-12-2022.csv", header = TRUE)

## PRICES, TIME AND LENGHT ####################################################################################
Pt_EUR <- rev(EUROSTOX_data[,6])
Time_EUR <- rev(as.Date(as.character(EUROSTOX_data[,1]), "%d/%m/%Y"))
N_EUR <- length(Pt_EUR)
...
#It seems that the best distribution is the df 3, still two outlier observatons
...
#seem to better capture the higher tail of the distribution. These are usually our quantiles of interest
...
par(mfrow = c(1,2))
acf(EUR_fit@fit$z)
Box.test(EUR_fit@fit$z, lag = 10, type = c("Ljung-Box"))
plot(EUR_fit)
infocriteria(EUR_fit)

#ARMA(4,2) - GARCH(1,1) seems to be the best fit. second best ARMA(3,3) - GARCH(2,1)
