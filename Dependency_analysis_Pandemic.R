...
stdFit(z_EUR)
EUR_fit@fit$matcoef
std_z_EUR <- pstd(z_EUR, mean = mean(z_EUR), sd = sd(z_EUR) , nu = 3.75434559)  
hist(std_z_EUR, 
...
stdFit(z_SP)
SP_fit@fit$matcoef
std_z_SP <- pstd(z_SP, mean = mean(z_SP), sd = sd(z_SP) , nu = 7.464550)  
hist(std_z_SP,
...
set.seed(2013)
cpCop <- (cpCopula(U, b = 1, method = "nonseq"))
cpCop 
plot(U)
?cpCopula
...
## Extract parameter estimates
len_Gr <- length(coef(fitG_r))
p_Gr <- coef(fitG_r)[1]



#Survival-Gumbel copula MOME tau #################################################################################
library(Copula.surv)
e <- matrix(1, ncol = 742, nrow = 1)
param_SG <- U2.Gumbel(pobs(z_EUR), pobs(z_SP),e,e, lower = 0.01, upper = 10)
param_SG
testg  <- Test.Gumbel(pobs(z_EUR), pobs(z_SP),e,e, lower=0.63,upper=0.70, U.plot = FALSE)
testg
#Survival-Gumbel MOME rho#################################################################################################
fitSG_r <- (fitCopula(surviv, data = U, method = "irho")) ## The corresponding copula parameter estimate
summary(fitSG_r)
fitSG_r@copula@parameters

## Extract parameter estimates
len_SGr <- length(coef(fitSG_r))
p_SGr <- coef(fitSG_r)[1]
...
tc <- tCopula(param = p_tm, df = df_tm)
set.seed(271)
X <- rCopula(742, copula = tc) # sample from the t copula
